...
def hue(frame):
    ...
    original_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    ...
    #ISSUE is bitwise_and in segmented roi im combining frame + frame, then in dilated_contour function i believe I am actually dilating the mask of hue rather the contour this can lead to problems as the bright spots from camera can also be dialted. We may want to stick to contours only. 

    #Moving on after all that I am again combining dilating_contour which is a dilated mask of the segmented_roi and combining that with the original frame. 
    #Find numpy way to combine frames. 

    contours_passed = segmented_roi
    ...
    new_roi = cv2.bitwise_and(dilated_contour, frame)
    ...
    return transformation #or can return new_roi same thing?
