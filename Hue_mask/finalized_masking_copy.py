...
#Thought: takes the inverse of the mask created via hue and fill that in with black
...
def hue(frame):
    ...
    # Dilate the masked image for context for aruco detector 
    kernel_size = 90  # Size of the kernel
    ...
    # Create a black image of the same size as my frame
    result = np.zeros_like(frame)

    # Draw the contours on the result image
    cv2.drawContours(result, contours, -1, (255, 255, 255), thickness=cv2.FILLED)

    # Applying the inverse to orignal hue mask to select everything nonrelavant selecting everuthing else but the inside of mask
    inverse_mask = cv2.bitwise_not(mask)
    result = cv2.bitwise_and(frame, result, mask=inverse_mask)


    # here im adding the masked frame to the result image
...
def pre_proccessing(frame):
    ...
    new_trans = cv2.adaptiveThreshold(new_trans, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 43,2)
    
    #_, new_trans = cv2.threshold(new_trans, 90, 255, cv2.THRESH_BINARY)
