...
def transformation(frame):
    ...
    transformation = cv2.GaussianBlur(segmented_roi, (5, 5), 0)
    ...
    transformation = cv2.drawContours(transformation, contours, -1, (0, 255, 0), 2)
    ...
    eroded_img = cv2.erode(transformation, kernel, iterations=iterations, anchor=(2, 2))
    '''
...
def prossessing(transformation):
    new_trans = cv2.cvtColor(transformation, cv2.COLOR_BGR2GRAY)
    ...
    visualizer = cv2.cvtColor(new_trans, cv2.COLOR_GRAY2BGR)
    visualizer = cv2.aruco.drawDetectedMarkers(visualizer, corners, ids)
    return visualizer
...
'''
TEST RN NOTES 
testing with just contours not erosion
'''


#after meeting: They said not to do erosion do dilation. after dilation mask the dilation and process it to see it better.
#Also visualize how it looks with contour vs dilation vs both.


'''

TEST RN NOTES 
Gettting rid of erosion right now and dilating the image we will first see what this does and then pass it in the parameters function to post process it and see it again. 
