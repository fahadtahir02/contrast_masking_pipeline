...
def contour_and_mask(frame):
    ...
    _, binary_image = cv2.threshold(local_contrast, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    # binary_image = cv2.adaptiveThreshold(local_contrast, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, adaptive_threshold_value, 1)
...
adaptive_threshold_value = 45  # Initial value for adaptive thresholding
...
def update_adaptive_threshold(val):
    ...
    if adaptive_threshold_value % 2 == 0:
        adaptive_threshold_value += 1
...
while True:
    ...
    # proccessed = pre_proccessing(frame)

    # Call the contour_and_mask function to get the masked frame
    masked_frame = contour_and_mask(frame)

    # Calculate the local contrast of the original frame for display purposes
    contrast_show = calculate_local_contrast(frame)

    # Create a stacked image for display
    stacked = np.hstack((frame, masked_frame))
