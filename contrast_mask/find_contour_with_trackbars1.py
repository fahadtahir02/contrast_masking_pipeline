...
#Cont - distribution of pixel intensities in the image measuring dark and light pixel intensities. 
#Based on the lighting condition - find the pixel intensity for the aruco tag along with the frequency of how many pixels share the same intensity
#contour them and then mask that contour and return the mask
#run detection on the mask
...
while True:
    ...
    gray_frame = cv2.cvtColor(cropped_frame, cv2.COLOR_BGR2GRAY)#COLOR_BGR2GRAY
    ...
    #cv2.putText(cropped_frame, f'Contrast: {contrast:.2f}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
